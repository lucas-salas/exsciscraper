...
class QuizScraper:
    ...
    def get_quizzes_with_reports(self, quiz_search_terms: dict, course_designation, max_len=0):
        ...
        #     )
        import pickle
        with open("/Users/spleut/Projects/Coding/exsciscraper/resources/pickles/search_results.pkl", "rb") as file:
        ...
        with Pool(4) as pool:
            updated_quiz_dict['pre'] = pool.map(func=pre_rph.fetch_reports, iterable=[quiz for quiz in search_results['pre']])
            updated_quiz_dict['post'] = pool.map(func=post_rph.fetch_reports, iterable=[quiz for quiz in search_results['post']])
