...
def main():
    ...
    uwrs_no_score_pair = cleaner.clean_dfs(dirty_df_pair)
    pre_uwrs = uwrs_handler.translate_scores(uwrs_no_score_pair.pre)
    post_uwrs = uwrs_handler.translate_scores(uwrs_no_score_pair.post)
    ...
    reduced_demographics.insert(insert_ind, 'uid', cleaner.generate_uids(reduced_demographics))
    # Add uid col to uwrs df (insert to right of term code col)
    insert_ind = uwrs_no_demographics.columns.get_loc("term_code") + 1
    uwrs_no_demographics.insert(insert_ind, 'uid', cleaner.generate_uids(uwrs_no_demographics))
