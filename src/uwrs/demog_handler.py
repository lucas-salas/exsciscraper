...
def demographics_count_not_found(demographics_info, uwrs_df):
    ...
    tmp_demog_names = (
        demographics_info["First Name"] + " " + demographics_info["Last Name"]
    )
...
def concat_names(demographics_info):
    ...
    """
    return (
        demographics_info["First Name"].str.lower()
        + " "
        + demographics_info["Last Name"].str.lower()
    )
...
def create_canvas_style_section(demographics_info):
    ...
    """
    return (
        demographics_info["Course Identification"].str.slice(start=0, stop=4)
        + "-"
        + demographics_info["Course Identification"].str.slice(start=-4)
        + "-"
        + demographics_info["Course Section"].astype("str")
    )
...
def build_semi_final_df(uwrs_no_demographics, reduced_demographics):
    ...
    demog_columns = [
        col
        for col in reduced_demographics.columns.tolist()
        if col not in dont_want_list
    ]
    row_iterator = uwrs_no_demographics.itertuples()
    series_list = []
    for row in row_iterator:
        # Find entries by uid that appear in both dataframes
        search_result_series = reduced_demographics.index[
            reduced_demographics["uid"] == row.uid
        ]
        ...
        if len(search_result) == 1:
            ...
            series_list.append(
                pd.concat([uwrs_series, demographics_series]).to_frame().transpose()
            )
