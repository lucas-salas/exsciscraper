...
class QuizScraper:
    ...
    @staticmethod
    def _filter_courses(master_course_list, course_designation) -> list[canvasapi.course.Course]:
    ...
    def get_quizzes_with_reports(self, quiz_search_terms: dict, course_designation):
        semester_course_list = self._get_account_courses()
        filtered_course_list = self._filter_courses(semester_course_list, course_designation)
        search_results = {}
        for pre_post in quiz_search_terms.keys():
            search_results[pre_post] = self._search_quizzes(filtered_course_list, quiz_search_terms[pre_post])
